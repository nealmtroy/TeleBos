...
export function MessagePane({
  ...
  const handleSendGif = (gifUrl: string) => {
    sendMutation.mutate({ text: gifUrl });
    setShowEmojiPicker(false);
  };
                ...
                {pickerTab === "sticker" && (
                  <div className="flex-1 flex flex-col min-h-0">
                    {stickerSetsData && stickerSetsData.length > 0 ? (
                      <>
                        <div className="flex gap-2 overflow-x-auto pb-2 border-b border-slate-100 dark:border-slate-800/80 mb-2 scrollbar-none flex-shrink-0">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scroll">
                      <div className="grid grid-cols-2 gap-2">
                        {MOCK_GIFS.filter((gif) =>
                          gifSearch ? gif.tags.toLowerCase().includes(gifSearch.toLowerCase()) : true
                        ).map((gif, idx) => (
                          <button
                            key={idx}
                            onClick={() => handleSendGif(gif.url)}
                            className="aspect-[4/3] rounded-xl overflow-hidden hover:opacity-90 active:scale-95 transition relative bg-slate-100 dark:bg-slate-800"
                          >
                            <img
                              src={gif.url}
