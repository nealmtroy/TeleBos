    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200/80">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-200/80 shadow-xs">
            ...
            <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded bg-primary-50 text-primary-700 border border-primary-200/60 ml-1">
              {user?.role || "BASIC"}
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-1.5 p-1 bg-slate-100/80 border border-slate-200/80 rounded-xl overflow-x-auto">
        ...
        >
          ...
          <span
            className={cn(
              "inline-flex items-center gap-1 px-1.5 py-0.2 rounded-full text-[10px] font-semibold",
              twoFactorEnabled
                  ...
                  <div className="pt-2 space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-1 text-xs text-slate-500">
                      <div className="flex items-center gap-1.5">
                        <span className={cn("h-3.5 w-3.5 rounded-full flex items-center justify-center text-[10px]", passwordCriteria.minLength ? "text-emerald-600" : "text-slate-300")}>
                          {passwordCriteria.minLength ? "✓" : "○"}
                        </span>
                        <span>Minimal 8 karakter</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className={cn("h-3.5 w-3.5 rounded-full flex items-center justify-center text-[10px]", passwordCriteria.hasUpperLower ? "text-emerald-600" : "text-slate-300")}>
                          {passwordCriteria.hasUpperLower ? "✓" : "○"}
                        </span>
                        <span>Huruf besar & kecil</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className={cn("h-3.5 w-3.5 rounded-full flex items-center justify-center text-[10px]", passwordCriteria.hasNumber ? "text-emerald-600" : "text-slate-300")}>
                          {passwordCriteria.hasNumber ? "✓" : "○"}
                        </span>
                        <span>Mengandung angka (0-9)</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className={cn("h-3.5 w-3.5 rounded-full flex items-center justify-center text-[10px]", passwordCriteria.hasSpecial ? "text-emerald-600" : "text-slate-300")}>
                      ...
                      <div className="space-y-1.5 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {key.scopes.map((s) => (
                            <span
                              key={s}
                              className="px-2 py-0.5 rounded text-[10px] font-mono font-medium bg-slate-100 text-slate-600"
                            >
                              {s}
                            </span>
                          ))}
